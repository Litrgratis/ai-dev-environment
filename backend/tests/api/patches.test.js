...
jest.mock("../../src/services/patchService.cjs", () => ({
  generatePatch: jest.fn(),
}));
import patchService from "../../src/services/patchService.cjs";
...
describe("Patch Routes", () => {
  ...
  it("should generate a patch for valid code and instruction", async () => {
    ...
    const mockResponse = {
      patch: mockPatch,
      description:
        'Patch generated for instruction: "change the log message to "hello patched world""',
    };
    ...
    expect(patchService.generatePatch).toHaveBeenCalledWith(
      originalCode,
      instruction,
    );
  });
  ...
  it("should return 500 if patch generation fails", async () => {
    ...
    jest
      .spyOn(patchService, "generatePatch")
