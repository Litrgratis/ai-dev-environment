...
describe('LLM Service Tests', () => {
  ...
  describe('AI Suggestions Generation', () => {
    test('should generate suggestions from Gemini API', async () => {
      ...
      const result = await completionService.generateAISuggestions(context, 'javascript');
    ...
    test('should handle API errors gracefully', async () => {
      ...
      await expect(completionService.generateAISuggestions(context, 'javascript'))
  ...
  describe('Fallback Logic', () => {
    test('should use mock suggestions when Gemini API is unavailable', async () => {
      ...
      const result = await completionService.generateSuggestions(context, 'javascript');
    ...
    test('should use mock suggestions in test environment', async () => {
      ...
      expect(result.suggestions).toEqual(['log()', 'error()', 'warn()', 'info()']);
      expect(result.fromCache).toBe(false);
    ...
    test('should provide fallback when AI service fails', async () => {
      ...
      expect(result.suggestions).toEqual(['function', 'const', 'let', 'var', 'if', 'for', 'while']);
  ...
  describe('Language Support', () => {
    test('should handle different programming languages', async () => {
      ...
      const result = await completionService.generateAISuggestions(context, 'python');
    ...
    test('should generate appropriate context for different languages', async () => {
      ...
      for (const language of languages) {
        ...
        const calledPrompt = mockModel.generateContent.mock.calls[mockModel.generateContent.mock.calls.length - 1][0];
  ...
  describe('Performance & Caching', () => {
    test('should cache successful responses', async () => {
      ...
      const result1 = await completionService.generateSuggestions(context, 'javascript');
      expect(result1.fromCache).toBe(false);

      // Second call - should hit cache (if Redis is available)
      const result2 = await completionService.generateSuggestions(context, 'javascript');
