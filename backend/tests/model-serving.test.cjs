...
let createModelServer;
let fetch;

tap.before(async () => {
  // Dynamically import ES modules
  const modelServing = await import("../src/inference/model-serving.ts");
  createModelServer = modelServing.createModelServer;
...
tap.test("GET /inference returns model info only", async (t) => {
  const response = await fetch(`${BASE_URL}/inference`, {
    method: "GET",
    headers: { "Content-Type": "application/json" },
  });
  ...
  t.match(
    ...
    "Response should contain only model info",
  );
});
