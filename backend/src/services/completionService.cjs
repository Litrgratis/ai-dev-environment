...
class CompletionService {
  ...
  async generateSuggestions(context, language = 'javascript') {
    try {
      // Check cache first
      const cached = await this.getCachedSuggestions(language, context.text || context);
      if (cached) {
      ...
      // Cache the results
      await this.cacheSuggestions(language, context.text || context, suggestions);
  ...
  async generateAISuggestions(context, language) {
    try {
      const { result: suggestions } = await profiler.profile('completion', async () => {
        const { text: contextText, cursorPosition, contextWindow } = context;
        const beforeCursor = contextText ? contextText.substring(0, cursorPosition) : contextWindow;
