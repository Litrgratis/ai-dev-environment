...
class CompletionController {
  ...
  async handleCompletion(request, reply) {
    ...
    try {
      const { context, language = 'javascript', maxSuggestions = 5 } = request.body;
      ...
      // Record metrics
      metricsHelpers.recordCompletionRequest(language, userId, result.fromCache, latency);
  ...
  async getCompletionStats(request, reply) {
    ...
    return {
      user: userId,
      stats: {
        totalRequests: metricsHelpers.getCacheStats().hits + metricsHelpers.getCacheStats().misses,
